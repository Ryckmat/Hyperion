...
class TestQualitySystemIntegration:
    ...
    def test_multiple_chat_requests_quality_tracking(self):
        ...
        try:
            ...
            metrics_response = requests.get(
                f"{self.api_base_url}/api/quality/metrics?hours=24", timeout=10
            )

            if metrics_response.status_code == 200:
                updated_metrics = metrics_response.json()
                # Note: Le nombre peut varier selon l'état de l'API et configuration
                assert "total_responses" in updated_metrics
                assert isinstance(updated_metrics["total_responses"], int)
                print(
                    f"✅ Métriques accessibles: {updated_metrics['total_responses']} réponses trackées"
                )
...
class TestMasterScriptIntegration:
    ...
    def test_quality_modules_importable(self):
        ...
        try:
            ...
            from src.hyperion.modules.rag.quality.hallucination_detector import (  # noqa: F401
